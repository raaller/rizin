 ...
 */
// The routines below are written with the opbuilder shorthand rather than the
// raw constructors, so the macro set is opened here and closed after them.
#include <rz_il/rz_il_opbuilder_begin.h>

/**
 * \brief Reverse the bit order of a 32-bit value.
 *
 * \param x The value, consumed by the returned expression.
 *
 * \return A pure expression where bit i of the result is bit 31 - i of \p x.
 */
RZ_API RZ_OWN RzILOpBitVector *rz_il_bitrev32(RZ_BORROW RzILOpBitVector *x) {
 ...
 */
RZ_API RZ_OWN RzILOpBitVector *rz_il_popcount_bytes32(RZ_BORROW RzILOpBitVector *x) {
	return LET("_b0", x,
 ...
 */
RZ_API RZ_OWN RzILOpBitVector *rz_il_deinterleave32(RZ_BORROW RzILOpBitVector *x) {
 ...
 */
RZ_API RZ_OWN RzILOpBitVector *rz_il_interleave32(RZ_BORROW RzILOpBitVector *x) {
