...
static void decode_3op(C6xInsn *insn, const C6xRow *row) {
	...
	if (nops == 3 && insn->ops[0].kind == C6X_OP_IMM && insn->ops[0].v.imm.value == 0 &&
		(is_or || is_add || is_sub)) {
		...
		insn->ops[0].v.imm.value < 0) {
		set_ins(insn, C6X_INS_SUB); // ADD -k, src2, dst  ==  SUB src2, k, dst
		insn->op_type = RZ_ANALYSIS_OP_TYPE_SUB;
		st64 k = -insn->ops[0].v.imm.value;
		insn->ops[0] = insn->ops[1];
		op_imm(&insn->ops[1], k);
	} else if (nops == 3 && is_sub && insn->ops[0].kind == C6X_OP_REG &&
		insn->ops[1].kind == C6X_OP_REG && insn->ops[0].v.reg.num == insn->ops[1].v.reg.num &&
		insn->ops[0].v.reg.side == insn->ops[1].v.reg.side) {
...
static void decode_darith(C6xInsn *insn, const C6xRow *row) {
	...
	if (insn->ops[1].kind == C6X_OP_IMM && insn->ops[1].v.imm.value == 0 && (d_add || d_sub)) {
		set_ins(insn, d_sub ? C6X_INS_NEG : C6X_INS_MV);
		insn->op_type = RZ_ANALYSIS_OP_TYPE_MOV;
		insn->ops[1] = insn->ops[2]; // dst follows src
		insn->nops = 2;
	} else if (insn->ops[1].kind == C6X_OP_IMM && insn->ops[1].v.imm.value == -1 && d_xor) {
		set_ins(insn, C6X_INS_NOT); // XOR src, -1, dst  ==  NOT src, dst
		insn->op_type = RZ_ANALYSIS_OP_TYPE_NOT;
		insn->ops[1] = insn->ops[2];
		insn->nops = 2;
	} else if (d_add && insn->ops[1].kind == C6X_OP_IMM && insn->ops[1].v.imm.value < 0) {
		set_ins(insn, C6X_INS_SUB); // ADD src2, -k, dst  ==  SUB src2, k, dst
		insn->op_type = RZ_ANALYSIS_OP_TYPE_SUB;
		op_imm(&insn->ops[1], -insn->ops[1].v.imm.value);
	} else if (d_sub && insn->ops[0].kind == C6X_OP_REG && insn->ops[1].kind == C6X_OP_REG &&
		insn->ops[0].v.reg.num == insn->ops[1].v.reg.num && insn->ops[0].v.reg.side == insn->ops[1].v.reg.side) {
		set_ins(insn, C6X_INS_ZERO); // SUB src, src, dst  ==  ZERO dst
		insn->op_type = RZ_ANALYSIS_OP_TYPE_MOV;
		insn->ops[0] = insn->ops[2];
		insn->nops = 1;
...
static void decode_branch(C6xInsn *insn) {
	...
	insn->ops[0].v.imm.value = disp; // absolute target resolved by the analysis filler
	insn->nops = 1;
	...
	if (insn->creg == 0 && insn->z) {
		...
		op_reg(&insn->ops[1], insn->unit_side, 3); // A3 on .S1, B3 on .S2
		insn->nops = 2;
		return;
	}
...
static void decode_branch_nop(C6xInsn *insn) {
	...
	insn->unit_side = BIT(w, 1);
	insn->ops[0].kind = C6X_OP_PCREL;
	insn->ops[0].v.imm.value = sext(BITS(w, 27, 16), 12) * 4;
	op_imm(&insn->ops[1], n);
	insn->nops = 2;
}
...
static void decode_bdec(C6xInsn *insn) {
	...
	insn->unit_side = s;
	insn->ops[0].kind = C6X_OP_PCREL;
	insn->ops[0].v.imm.value = sext(BITS(w, 22, 13), 10) * 4;
...
static bool nounit_apply(C6xInsn *insn, const C6xNoUnitRow *r, ut32 w) {
	...
	case NOUNIT_II:
		...
		break;
	case NOUNIT_STGCYC:
		op_imm(&insn->ops[0], BITS(w, 27, 22));
		insn->nops = 1;
		break;
	case NOUNIT_UNITS:
		insn->ops[0].kind = C6X_OP_UNITMASK;
		insn->ops[0].v.units = BITS(w, 25, 18);
		insn->nops = 1;
		break;
	default:
		break;
...
static bool decode_unit(const C6xArchDesc *d, C6xInsn *insn) {
	...
	if (BITS(w, 4, 2) == 0x6) {
		...
		if (BITS(w, 31, 29) == 0 && BIT(w, 28) == 1 && feat_ok(d, FEAT_C66X) && (BITS(w, 11, 5) == 0x03 || BITS(w, 11, 5) == 0x07)) {
			bool is_sub = BITS(w, 11, 5) == 0x07;
			ut8 s = BIT(w, 1);
			ut8 x = BIT(w, 12);
			...
			insn->unit_side = s;
			insn->cross = x != 0;
			op_regpair(&insn->ops[0], s, BITS(w, 17, 13)); // src1
			op_regpair(&insn->ops[1], x ? !s : s, BITS(w, 22, 18)); // src2 (cross-path)
			op_regpair(&insn->ops[2], s, BITS(w, 27, 23)); // dst
			insn->nops = 3;
			return true;
		}
	...
	if (feat_ok(d, FEAT_SIMD) && BITS(w, 5, 2) == 0xc && BITS(w, 11, 10) <= 1) {
		...
		for (size_t i = 0; i < RZ_ARRAY_SIZE(c6x_m_ext_rows); i++) {
			...
			if (r->swap) {
				...
				if (r->src1_cst) {
					op_imm(&insn->ops[1], BITS(w, 17, 13));
				} else {
...
static void decode_compact(C6xInsn *insn, ut16 w, bool rs, bool sat, ut8 dsz, bool br) {
	...
	if ((w & 0x6) == 0x6) {
		...
		if (((w >> 1) & 0xf) == 0xf) {
			...
			insn->unit = C6X_UNIT_M;
			insn->cross = x;
			compact_reg(&insn->ops[0], (w >> 13) & 0x7, s, rs);
			compact_reg(&insn->ops[1], (w >> 7) & 0x7, x ? !s : s, rs);
			compact_reg(&insn->ops[2], (ut8)(((w >> 10) & 0x3) * 2), s, rs);
			insn->nops = 3;
			return;
		}
		...
		if ((((w >> 4) & 0x7) == 0x6 || ((w >> 4) & 0x7) == 0x7) && ((w >> 10) & 0x7) == 0x6) {
			...
			case 6:
				...
				insn->op_type = RZ_ANALYSIS_OP_TYPE_MOV;
				insn->ops[0] = reg;
				insn->ops[1].kind = C6X_OP_CTRLREG;
				insn->ops[1].v.ctrl = "ilc";
				insn->nops = 2;
				return;
			default:
		...
		if (((w >> 1) & 0x3f) == 0x33 && ((w >> 10) & 0x3) == 0x3 && !((w >> 13) & 1) && ((w >> 12) & 1)) {
			...
			insn->unit = C6X_UNIT_NONE;
			insn->unit_side = 0;
			op_imm(&insn->ops[0], (((w >> 7) & 1)) | (((w >> 8) & 1) << 1) | (((w >> 9) & 1) << 2) | (((w >> 14) & 1) << 3) | (((w >> 15) & 1) << 4) | ((w & 1) << 5));
			insn->nops = 1;
			return;
		}
		if (((w >> 1) & 0x3f) == 0x33 && ((w >> 10) & 0x3) == 0x3 && !((w >> 13) & 1) && !((w >> 12) & 1)) {
			...
			op_imm(&insn->ops[0], 1 + (((w >> 7) & 1) | (((w >> 8) & 1) << 1) | (((w >> 9) & 1) << 2) | (((w >> 14) & 1) << 3)));
			insn->nops = 1;
			return;
		}
		if (((w >> 1) & 0x3f) == 0x33 && ((w >> 10) & 0x3) == 0x3 && ((w >> 13) & 1)) {
			set_ins(insn, ((w >> 12) & 1) ? C6X_INS_SPMASKR : C6X_INS_SPMASK);
			insn->op_type = RZ_ANALYSIS_OP_TYPE_NULL;
			insn->unit = C6X_UNIT_NONE;
			insn->unit_side = 0;
			insn->ops[0].kind = C6X_OP_UNITMASK;
			insn->ops[0].v.units = (ut8)((w & 1) | (((w >> 7) & 1) << 1) | (((w >> 8) & 1) << 2) |
		...
		if (((w >> 1) & 0x3f) == 0x17 && ((w >> 10) & 1)) {
			set_ins(insn, C6X_INS_ADDK);
			insn->op_type = RZ_ANALYSIS_OP_TYPE_ADD;
			insn->unit = C6X_UNIT_S;
			op_imm(&insn->ops[0], (((w >> 13) & 1)) | (((w >> 14) & 1) << 1) | (((w >> 15) & 1) << 2) | (((w >> 11) & 1) << 3) | (((w >> 12) & 1) << 4));
		...
		if (((w >> 1) & 0x3f) == 0x3b && ((w >> 10) & 0x3) == 0x3) {
			set_ins(insn, ((w >> 7) & 1) ? C6X_INS_SUBAW : C6X_INS_ADDAW);
			insn->op_type = ((w >> 7) & 1) ? RZ_ANALYSIS_OP_TYPE_SUB : RZ_ANALYSIS_OP_TYPE_ADD;
			insn->unit = C6X_UNIT_D;
			insn->unit_side = s;
			op_reg(&insn->ops[0], 1, 15); // B15
			op_imm(&insn->ops[1], (((w >> 13) & 1)) | (((w >> 14) & 1) << 1) | (((w >> 15) & 1) << 2) | (((w >> 8) & 1) << 3) | (((w >> 9) & 1) << 4));
			op_reg(&insn->ops[2], 1, 15); // B15
			insn->nops = 3;
			return;
		}
		if (((w >> 1) & 0x3f) == 0x1b) {
			...
			insn->cross = s == 0;
			op_reg(&insn->ops[0], 1, 15); // B15
			op_imm(&insn->ops[1], (((w >> 13) & 0x1) << 0) | (((w >> 14) & 0x1) << 1) | (((w >> 15) & 0x1) << 2) | (((w >> 11) & 0x1) << 3) | (((w >> 12) & 0x1) << 4));
			compact_reg(&insn->ops[2], (w >> 7) & 0x7, s, rs);
			insn->nops = 3;
			return;
		}
		ut8 unit_f = (w >> 3) & 0x3;
	...
	if ((w & 0x6) == 0x4) {
		...
		bool dstk = ((w >> 10) & 0x3) == 0x3 && (w & 0x8000);
		ut8 sz = dstk ? 0 : (w >> 9) & 1;
		bool ld = (w >> 3) & 1;
	...
	if ((w & 0x6) == 0x2) {
		...
		if (!br && ((w >> 1) & 0x7) == 0x5 && !((w >> 10) & 1)) {
			...
			compact_reg(&insn->ops[0], (w >> 13) & 0x7, s, rs);
			compact_reg(&insn->ops[1], (w >> 7) & 0x7, x ? !s : s, rs);
			compact_reg(&insn->ops[2], (w >> 4) & 0x7, s, rs);
			insn->nops = 3;
			return;
		}
		...
		if (br && ((w >> 3) & 1)) {
			...
			if (b5 == 0 && b4 == 1) { // Scs10: CALLP scst10, 5
				...
				insn->op_type = RZ_ANALYSIS_OP_TYPE_CALL;
				insn->ops[0].kind = C6X_OP_PCREL;
				insn->ops[0].v.imm.value = sext((w >> 6) & 0x3ff, 10) * 2;
				insn->nops = 1;
				return;
			}
...
RZ_IPI ut8 c6x_branch_slots(const C6xInsn *insn) {
	...
	case C6X_INS_BNOP: {
		...
		ut8 filled = insn->nops ? (ut8)insn->ops[insn->nops - 1].v.imm.value : 0;
