		...
		for (size_t i = 0; i < n; i++) {
			for (ut8 o = 0; o + 1 < ins[i].nops; o++) {
				...
				if (c6x_same_reg(s, reg)) {
					*end = p.slots[p.n - 1].addr + p.slots[p.n - 1].size;
					return true;
				}
			}
		}
		at = p.slots[p.n - 1].addr + p.slots[p.n - 1].size;
	}
	return false;
}
...
static RzILOpEffect *c6x_lift_latency_window(RzAnalysis *analysis, const C6xArchDesc *c6x,
	...
	ut32 hold = 0;
	ut64 window_end = 0;
	ut64 after = pkt->slots[pkt->n - 1].addr + pkt->slots[pkt->n - 1].size;
	for (size_t i = 0; i < n; i++) {
...
static RzILOpEffect *c6x_lift_il(RzAnalysis *analysis, const C6xArchDesc *c6x,
	...
	ut64 next = pkt.slots[pkt.n - 1].addr + pkt.slots[pkt.n - 1].size;
	for (ut8 k = 1; k <= bslots; k++) {
		...
		next = dp.slots[dp.n - 1].addr + dp.slots[dp.n - 1].size;
	}
