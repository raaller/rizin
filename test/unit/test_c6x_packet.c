...
#define FP_BASE 0xc0c009e0

static bool read_compact(void *user, ut64 addr, ut8 *buf, size_t len) {
	if (addr < FP_BASE || addr + len > FP_BASE + sizeof(compact_fp)) {
		return false; // only the one fetch packet is mapped
	}
	memcpy(buf, compact_fp + (addr - FP_BASE), len);
	return true;
}
